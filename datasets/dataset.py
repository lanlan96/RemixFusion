...
def get_dataset(config):
    ...
    elif config['dataset'] == 'Fast_syn':
        dataset = FastsynDataset
    
    elif config['dataset'] == 'Largeindoor':
        dataset = LargeindoorDataset

    elif config['dataset'] == 'uhumans':
        dataset = uhumansDataset
    
    elif config['dataset'] == 'BS3D':
        dataset = BS3DDataset
...
class BaseDataset(Dataset):
    def __init__(self, cfg):
        ...
        if cfg['data']['downsample']>1:
            ...
            self.fx, self.fy =  cfg['cam']['fx'],\
                cfg['cam']['fy']
            self.cx, self.cy = cfg['cam']['cx'],\
                cfg['cam']['cy']
        self.distortion = np.array(
            cfg['cam']['distortion']) if 'distortion' in cfg['cam'] else None
        self.crop_edge = cfg['cam']['crop_edge'] if 'crop_edge' in cfg['cam'] else 0
        self.ignore_w = cfg['tracking']['ignore_edge_W']
        self.ignore_h = cfg['tracking']['ignore_edge_H']
        if 'crop_size' in cfg['cam']:
            self.total_pixels = (cfg['cam']['crop_size'][0]) * (cfg['cam']['crop_size'][1])
        else:
            self.total_pixels = (self.H - self.crop_edge*2) * (self.W - self.crop_edge*2)
...
class ReplicaDataset(BaseDataset):
    def __init__(self, cfg, basedir, trainskip=1, 
        ...
        if cfg['cam']['crop_edge'] > 0:
            ...
            cfg['cam']["H"] -= cfg['cam']['crop_edge']*2
            cfg['cam']["W"] -= cfg['cam']['crop_edge']*2
...
class FastsynDataset(BaseDataset):
    def __init__(self, cfg, basedir, trainskip=1, 
                 downsample_factor=1, translation=0.0, 
                 sc_factor=1., crop=0):
        super(FastsynDataset, self).__init__(cfg)
...
class LargeindoorDataset(BaseDataset):
    def __init__(self, cfg, basedir, trainskip=1, 
                 downsample_factor=1, translation=0.0, 
                 sc_factor=1., crop=0):
        super(LargeindoorDataset, self).__init__(cfg)
        ...
        if os.path.join(
            self.basedir, 'color', '0.png') in glob.glob(os.path.join(
            self.basedir, 'color', '*.png')) :
            self.img_files = sorted(glob.glob(os.path.join(
                self.basedir, 'color', '*.png')), key=lambda x: int(os.path.basename(x)[:-4]))
            self.depth_paths = sorted(
                glob.glob(os.path.join(
            ...
            associations_path = os.path.join(self.basedir, 'associations.txt')
            if os.path.exists(associations_path):
                self.associations=np.loadtxt(associations_path,dtype=str)
                self.frame_ids = range(0, self.associations.shape[0])


        
        #Pose
        if os.path.exists(os.path.join(self.basedir, 'pose')):
            self.load_poses(os.path.join(self.basedir, 'pose'))
        else:
            unit_matrix = np.eye(4)
            
            result = np.tile(unit_matrix, (self.associations.shape[0], 1, 1))
            self.poses = torch.from_numpy(result).float()
    ...
    def __getitem__(self, index):
        ...
        if self.associations is None:
            ...
            color_path = os.path.join(self.basedir,self.associations[index][1])
            depth_path = os.path.join(self.basedir,self.associations[index][3])
            if 'Kaile' in self.basedir:
                color_path = os.path.join(
                self.basedir,self.associations[index][1])
                depth_path = os.path.join(self.basedir,self.associations[index][3])
...
class BS3DDataset(BaseDataset):
    def __init__(self, cfg, basedir, trainskip=1, 
                 downsample_factor=1, translation=0.0, 
                 sc_factor=1., crop=0):
        super(BS3DDataset, self).__init__(cfg)
        ...
        self.crop = crop
        if os.path.join(
            self.basedir, 'color', '0.jpg') in glob.glob(os.path.join(
            self.basedir, 'color', '*.jpg')) :
            self.img_files = sorted(glob.glob(os.path.join(
        ...
        if 'crop_size' in cfg['cam']:
            self.H_out = self.config['cam']['crop_size'][0]
            self.W_out = self.config['cam']['crop_size'][1]
            H_out_with_edge, W_out_with_edge = self.H_out + self.config['cam']['crop_edge'] * 2, self.W_out + self.config['cam']['crop_edge'] * 2
            
            self.fx *= W_out_with_edge / self.W
            self.fy *= H_out_with_edge / self.H
            self.cx *= W_out_with_edge / self.W
            self.cy *= H_out_with_edge / self.H
            
            self.H = H_out_with_edge - self.config['cam']['crop_edge']*2
            self.W = W_out_with_edge - self.config['cam']['crop_edge']*2
            
            self.cx -= self.config['cam']['crop_edge']
            self.cy -= self.config['cam']['crop_edge']

            # print("fx",self.fx,self.fy,self.cx,self.cy)
    ...
    def __getitem__(self, index):
        ...
        depth_data = depth_data.astype(np.float32) / self.png_depth_scale * self.sc_factor
        H, W = depth_data.shape
        depth_data = torch.from_numpy(depth_data.astype(np.float32)).float()
    ...
    def load_poses(self, path):
        self.poses = []
        poses = np.loadtxt(path, dtype=np.float)
        poses = poses[:,1:]
...
class TUMDataset(BaseDataset):
    def __init__(self, cfg, basedir, align=True, trainskip=1, 
        ...
        valid_num = len(self.poses)

        gt_poses_torch = torch.cat(self.poses)
        gt_poses_torch = gt_poses_torch.reshape(valid_num,4,4)
        ...
        self.config['cam']["H"] =self.H
        self.config['cam']["W"] =self.W
        self.config['cam']["fx"] =self.fx
        self.config['cam']["fy"] =self.fy
        self.config['cam']["cx"] =self.cx
        self.config['cam']["cy"] =self.cy
...
class uhumansDataset(BaseDataset):
    def __init__(self, cfg, basedir, align=True, trainskip=1, 
                 downsample_factor=1, translation=0.0, 
                 sc_factor=1., crop=0, load=True):
        super(uhumansDataset, self).__init__(cfg)
    ...
    def loadtum(self, datapath, frame_rate=-1):
        ...
        image_list = os.path.join(datapath, 'color.txt')
        depth_list = os.path.join(datapath, 'depth.txt')
        ...
        for ix in range(0,image_data.shape[0]):
            i=j=k=ix
            images += [os.path.join(datapath, image_data[i, 1])]
            depths += [os.path.join(datapath, depth_data[j, 1])]
            c2w = self.pose_matrix_from_quaternion(pose_vecs[k])
